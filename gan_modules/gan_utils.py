...
class AdaptiveDA(nn.Module):
    def __init__(self, frequency=4, threshold=0.6, device="cpu"):
        super().__init__()
        self.prob = torch.tensor(0., device=device)
    ...
    def forward(self, x, target=False):
        if(target):
            self.n += 1
        prob = self.prob.item()
        is_applying_list = np.random.choice([0, 1], size=len(self.functions), p=[1 - prob, prob])
        for function, is_applying in zip(self.functions, is_applying_list):
            if(is_applying):
                x = function(x)
        return x
